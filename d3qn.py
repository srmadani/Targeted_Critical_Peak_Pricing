...
def evaluate_policy(env, model):
	scores = 0
	for j in range(3):
		s = env.reset(mode = 'validation', scale = True, set_num = j)
		done = False
		while not done:
...
parser.add_argument('--EnvIdex', type=int, default=0, help='Homes_D3QN')
...
parser.add_argument('--Max_train_steps', type=int, default=int(4e6), help='Max training steps')
...
parser.add_argument('--exp_noise_end', type=float, default=0.05, help='final explore noise')
parser.add_argument('--noise_decay_steps', type=int, default=int(1e6), help='decay steps of explore noise')
...
def main():
	...
	if opt.render:
		score, _ = evaluate_policy(eval_env, model)
		...
		total_steps, acts, rew, best_score, scores_hist = 0, [], [], -np.inf, []
		while total_steps < opt.Max_train_steps:
			...
			while True:
				...
				if (total_steps) % opt.eval_interval == 0:
					...
					print('EnvName:',BriefEnvName[opt.EnvIdex],'seed:',opt.seed,'steps: {}k'.format(int(total_steps/1000)),'score:', score)
		...
		with open(f'res/score_hist_{EnvName[opt.EnvIdex]}.pkl', 'wb') as file:
