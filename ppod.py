...
def evaluate_policy(env, agent):
	total_scores = 0
	for j in range(3):
		s = env.reset(mode = 'validation', set_num = j)
		done = False
		while not done:
			...
			total_scores += r
			s = s_next
	return total_scores/3, env.acts
...
parser.add_argument('--EnvIdex', type=int, default=0, help='Homes_PPOD')
...
parser.add_argument('--entropy_coef', type=float, default=5e-3, help='Entropy coefficient of Actor')
...
def main():
	...
	if opt.render:
		ep_r, acts = evaluate_policy(env, agent)
		...
		while total_steps < opt.Max_train_steps:
			...
			while not done:
				...
				if total_steps % opt.eval_interval == 0:
					score, act = evaluate_policy(eval_env, agent) # evaluate the policy for 3 times, and get averaged result
					rew.append(score)
